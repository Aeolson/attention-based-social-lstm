'''
Vanilla LSTM with encoder-decoder structure (seq2seq), only use basic observation states [x, y, v]
...
class Model_VanillaLSTM(nn.Module):
    def __init__(self, train_flag=True) -> None:
        super(Model_VanillaLSTM, self).__init__()
        ...
        self.inupt_emb = nn.Linear(in_features=2, out_features=64)
    ...
    def forward(self, pose_tags:Tensor, hist_tags:Tensor, pose_nghs:Tensor, hist_nghs:Tensor):
        ...
        tens_inp_sta = pose_tags.clone()                                                # n_batch x n_seq x n_in_feature
...
def train(train_set:Dataset, valid_set:Dataset, learn_rate=0.01):
    ...
    mdl = Model_VanillaLSTM().to(args['use_device'])
...
def predict(test_set:Dataset):
    mdl = Model_VanillaLSTM().to(args['use_device'])
    pred_test, _ = evaluate(mdl, test_set)
    return pred_test

def evaluate(mdl:Model_VanillaLSTM, eva_set:Dataset):
    eva_pred_tags = []
    eva_loss = 0
    cnt_samples = 0
...
def runinfo(mdl:Model_VanillaLSTM, eva_set:Dataset):
    avg_time = 0
    mdl.train_flag = False
