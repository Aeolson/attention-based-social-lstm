...
class Model_AttentionSocial_LSTM_F(nn.Module):
    def __init__(self, train_flag=True) -> None:
        super(Model_AttentionSocial_LSTM_F, self).__init__()
        ...
        # input embeding
        self.inp_emb_dim = 64
        self.inp_emb = nn.Linear(in_features=6, out_features=self.inp_emb_dim)
...
def train(train_set:Dataset, valid_set:Dataset, learn_rate=0.001):
    ...
    mdl = Model_AttentionSocial_LSTM_F().to(args['use_device'])
...
def evaluate(mdl:Model_AttentionSocial_LSTM_F, eva_set:Dataset):
    eva_pred_tags = []
    eva_loss = 0
    cnt_samples = 0
...
def runinfo(mdl:Model_AttentionSocial_LSTM_F, eva_set:Dataset):
    avg_time = 0
    mdl.train_flag = False
