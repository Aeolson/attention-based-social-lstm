'''
Convolutional social pooling LSTM, Without using maneuvers
reference paper:
    N. Deo, M. Trivedi. Convolutional social pooling for vehicle trajectory prediction. 2018 CVPRW.
'''
import sys
import os
import random
...
class Model_ConvSocial_LSTM(nn.Module):
    def __init__(self, train_flag=True) -> None:
        super(Model_ConvSocial_LSTM, self).__init__()
        ...
        self.inp_emb = torch.nn.Linear(in_features=2, out_features=32)
        self.enc_lstm = torch.nn.LSTM(input_size=self.inp_emb.out_features, hidden_size=64, batch_first=True)
        self.dyn_emb = torch.nn.Linear(in_features=self.enc_lstm.hidden_size, out_features=32)

        self.soc_conv_3x2 = torch.nn.Conv2d(in_channels=self.enc_lstm.hidden_size, out_channels=64, kernel_size=(3,2))
        self.soc_conv_3x1 = torch.nn.Conv2d(in_channels=self.soc_conv_3x2.out_channels, out_channels=16, kernel_size=(3,1))
        self.soc_maxpool = torch.nn.MaxPool2d(kernel_size=(2,1), padding=(1,0))

        soc_cat_size = (self.soc_map_size[0] - 2 - 2 + 1) // 2 * self.soc_conv_3x1.out_channels + self.dyn_emb.out_features
        self.dec_lstm = torch.nn.LSTM(input_size=soc_cat_size, hidden_size=128, batch_first=True)
    ...
    def forward(self, pose_tags:Tensor, hist_tags:Tensor, pose_nghs:Tensor, hist_nghs:Tensor):
        ...
        tens_soc_map = tens_soc_map.permute(0, 3, 1, 2)                     # n_batch x 64 x (h, w)
        
        # convolution social pooling
        tens_soc_out = self.leaky_relu(self.soc_conv_3x2(tens_soc_map))     # n_batch x 64 x (h-2, 1)
        tens_soc_out = self.leaky_relu(self.soc_conv_3x1(tens_soc_out))     # n_batch x 64 x (h-4, 1)
        tens_soc_out = self.soc_maxpool(tens_soc_out)                       # n_batch x 64 x ((h-3)//2
        tens_soc_out = tens_soc_out.reshape(n_batch, -1)                    # n_batch x 64*(h-3)//2

        tens_dyn_emb = self.leaky_relu(self.dyn_emb(tens_enc_out[:,0,:]))   # n_batch x 32
        tens_cat_enc = torch.cat((tens_soc_out, tens_dyn_emb), dim=-1)      # n_batch x 64*(h-3)//2+32

        # lstm decoder
        tens_dec_inp = tens_cat_enc.repeat(args['pred_seqlen'], 1, 1).permute(1,0,2)  # n_batch x n_pred_seq x 64*(h-3)//2+32
        tens_dec_out, (_, _) = self.dec_lstm(tens_dec_inp)
        tens_dec_out = self.dec_output(tens_dec_out)                                # n_batch x n_pred_seq x 2
...
def train(train_set:Dataset, valid_set:Dataset, learn_rate=0.01):
    ...
    mdl = Model_ConvSocial_LSTM().to(args['use_device'])
    ...
    for e in range(2000):
        ...
        for k in range(train_set.total_batch_num):
            ...
            loss_ = lossfuc.forward(pred_tags, futu_tags)
        ...
        #------------------------------------------------------------------------------------
        _, val_loss = evaluate(mdl, valid_set)
        print("epoch = %d, lr = %.6f, loss = %.4f"%(mdl.epoch, optimizer.param_groups[0]['lr'], val_loss))
...
def evaluate(mdl:Model_ConvSocial_LSTM, eva_set:Dataset):
    eva_pred_tags = []
    eva_loss = 0
    cnt_samples = 0
...
def runinfo(mdl:Model_ConvSocial_LSTM, eva_set:Dataset):
    avg_time = 0
    mdl.train_flag = False
