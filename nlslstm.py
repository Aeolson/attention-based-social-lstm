...
class Model_NoLocalSocial_LSTM(nn.Module):
    def __init__(self, train_flag=True) -> None:
        super(Model_NoLocalSocial_LSTM, self).__init__()
...
def train(train_set:Dataset, valid_set:Dataset, start_epoch=0, learn_rate=0.001):
    ...
    mdl = Model_NoLocalSocial_LSTM().to(args['use_device'])
...
def evaluate(mdl:Model_NoLocalSocial_LSTM, eva_set:Dataset):
    eva_pred_tags = []
    eva_loss = 0
    cnt_samples = 0
...
def runinfo(mdl:Model_NoLocalSocial_LSTM, eva_set:Dataset):
    avg_time = 0
    mdl.train_flag = False
